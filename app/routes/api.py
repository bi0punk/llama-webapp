...
def api_playground_chat_stream(request: Request, payload: dict[str, Any]) -> StreamingResponse:
    ...
    from app.services.chat_service import build_chat_request

    target = build_chat_request(request, payload, stream=True)
...
async def api_playground_chat(request: Request, payload: dict[str, Any]) -> JSONResponse:
    ...
    target = build_chat_request(request, payload, stream=False)
    base_url = target["base_url"]
    headers = target["headers"]
    body = target["body"]
