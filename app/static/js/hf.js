...
function downloadForm(repo, file) {
...
async function loadFiles(repo) {
    const container = getEl("hf-files");
    if (!container)
        return;
...
function init() {
    const input = getEl("hf-search-input");
    const button = getEl("hf-search-button");
    const box = getEl("hf-results");
    if (!input || !button || !box)
        return;
    const run = async () => {
